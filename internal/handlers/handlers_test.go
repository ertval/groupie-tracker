...
func TestHandlers_SearchHandler(t *testing.T) {
	...
	if response.Query != "Test" {
		t.Errorf("Expected query 'Test', got '%s'", response.Query)
	}
...
func TestHandlers_SuggestHandler(t *testing.T) {
	handlers := getTestHandlers()

	req, err := http.NewRequest("GET", "/suggest?q=Test", nil)
	if err != nil {
		t.Fatal(err)
	}

	rr := httptest.NewRecorder()
	handler := http.HandlerFunc(handlers.SuggestHandler)
...
func TestHandlers_SuggestHandler_ShortQuery(t *testing.T) {
	handlers := getTestHandlers()

	req, err := http.NewRequest("GET", "/suggest?q=T", nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestHandlers_RefreshHandler(t *testing.T) {
	store := getTestStore()
	service := service.NewService(store)
	apiClient := api.NewClient("https://groupietrackers.herokuapp.com", 10*time.Second) // Real API
	handlers := NewHandlers(store, service, apiClient)
	...
	rr := httptest.NewRecorder()
	handler := http.HandlerFunc(handlers.RefreshHandler)

	handler.ServeHTTP(rr, req)

	// Since we're using a real API that should work, expect 200
	if status := rr.Code; status != http.StatusOK {
		t.Errorf("Handler returned wrong status code: got %v want %v", status, http.StatusOK)
	}
...
func TestHandlers_RefreshHandler_WrongMethod(t *testing.T) {
	handlers := getTestHandlers()
