...
// TestRepository_FilterArtists_CreationDate tests filtering artists by creation date range using sliders
func TestRepository_FilterArtists_CreationDate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			results := repo.FilterArtists(params)
			...
			// Check specific artist names if provided
...
// TestRepository_FilterArtists_FirstAlbumYear tests filtering artists by first album year using sliders
func TestRepository_FilterArtists_FirstAlbumYear(t *testing.T) {
	tests := []struct {
		name     string
		fromYear *int
		toYear   *int
	...
	}{
		{
			...
			fromYear: intPtr(2010),
			toYear:   nil,
			wantMin:  4, // XXXTentacion, Juice WRLD, Alec Benjamin, Post Malone
		},
		{
			name:     "First album 1990-1992",
			fromYear: intPtr(1990),
			toYear:   intPtr(1992),
			wantMin:  1, // Red Hot Chili Peppers
		},
	}
...
// TestRepository_FilterArtists_MemberCounts tests filtering artists by member count using checkboxes
func TestRepository_FilterArtists_MemberCounts(t *testing.T) {
	tests := []struct {
		name         string
		memberCounts []int
		wantMin      int
	}{
		{
			...
			memberCounts: []int{1},
			wantMin:      2, // Expect at least 2 solo artists
		},
		{
			name:         "Small bands (2-4 members)",
			memberCounts: []int{2, 3, 4},
			wantMin:      5, // Should have many small bands
		},
		{
			name:         "Exactly 7 members",
			memberCounts: []int{7},
			wantMin:      1, // Queen should have 7 members
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Verify that all returned artists have member counts in the allowed list
...
// TestRepository_FilterArtists_Countries tests filtering artists by countries using checkboxes
func TestRepository_FilterArtists_Countries(t *testing.T) {
	tests := []struct {
		name      string
		countries []string
		wantMin   int
	}{
		{
			...
			wantMin:   10, // Many artists should have USA concerts
		},
		{
			name:      "UK concerts only",
			countries: []string{"UK"},
			wantMin:   2, // Several artists should have UK concerts
		},
		{
			name:      "USA or UK concerts",
			countries: []string{"USA", "UK"},
			wantMin:   12, // Should be many artists with USA or UK concerts
		},
	}
...
// TestRepository_FilterArtists_Combined tests combining multiple filters
func TestRepository_FilterArtists_Combined(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantMin: 0, // May not have solo artists in that range
		},
		{
			...
			wantMin: 0, // May not match due to data constraints
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			results := repo.FilterArtists(tt.params)
...
func TestRepository_GetFilterOptions(t *testing.T) {
	...
	// Should contain common countries like USA
	if !contains(options.Countries, "USA") {
		t.Error("Expected USA to be in countries list")
	}

	t.Logf("Filter options: Creation years %d-%d, Album years %d-%d, Member counts %v, Countries %d",
		options.CreationYearMin, options.CreationYearMax,
		options.FirstAlbumYearMin, options.FirstAlbumYearMax,
		options.MemberCounts, len(options.Countries))
}
